...
class RadarWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        fun schedulePeriodicUpdate(context: Context) {
            val workRequest =
                PeriodicWorkRequestBuilder<RadarWidgetWorker>(
                    15,
                    TimeUnit.MINUTES,
                ).build()
        ...
        internal suspend fun updateWidgetSuspended(
            context: Context,
            appWidgetManager: AppWidgetManager,
            appWidgetId: Int,
        ) {
            ...
            val isPremium =
                com.example.rainradar.billing.BillingManager
            ...
            if (isPremium) {
                ...
                val currentFileReady =
                    DwdWmsClient.isFrameReady(context, currentFrameTime, base) ||
                ...
                locationName =
                    if (zoomMode == "location" && hasLocation) {
                        "Lokale Ansicht"
                    } else {
                        "Deutschland"
                    }
                ...
                if (activeCoords != null) {
                    ...
                    for (i in forecastIndices.indices) {
                        ...
                        val isReady =
                            DwdWmsClient.isFrameReady(context, time, base) ||
            ...
            val outputBmp =
                finalBmp ?: Bitmap.createBitmap(300, 300, Bitmap.Config.ARGB_8888).apply {
            ...
            val refreshPendingIntent =
                if (isPremium) {
                    val refreshIntent =
                        Intent(context, RadarWidgetProvider::class.java).apply {
            ...
            val toggleZoomIntent =
                Intent(context, RadarWidgetProvider::class.java).apply {
                    action = ACTION_TOGGLE_ZOOM
                }
            val toggleZoomPendingIntent =
                PendingIntent.getBroadcast(
                    context,
                    0,
                    toggleZoomIntent,
